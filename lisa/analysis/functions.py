...
class FunctionsAnalysis(TraceAnalysisBase):
    ...
    @TraceAnalysisBase.cache
    def df_calls(self, tag_df=None, thread_root_functions=None):
        """
        ...
        :param tag_df: Dataframe containing the tag event, which is used to tag
            ...
            be used as tag keys.
            Tags are inherited from from both parents and children, children
            getting the priority. This allows a leaf function to emit an event
            and use it for the whole path that lead to there. Equally, if a
            function emits a tag, all the children of this call will inherit
            the tag too. This allows a top-level function to tag a whole
            subtree at once.
    ...
    def compare_with_traces(self, others, **kwargs):
        ...
        :Other arguments: Same as :meth:`profile_stats`.
        """
        ref = self.trace
        ...
        def get_df(trace):
            df = trace.analysis.functions.df_calls()
            df = df.copy(deep=False)
    ...
    def profile_stats(self, tag_df=None, **kwargs):
        """
        ...
        .. note:: Recursive calls are treated as if they were inlined in their
            ...
            result.
        """
        df = self.df_calls(tag_df=tag_df)
        return self._profile_stats_from_df(df, ref_group=None, **kwargs)

    @staticmethod
    def _profile_stats_from_df(df, metric='self_time', functions=None, tags=None, ref_group=None, **kwargs):
        metrics = _CallGraphNode._METRICS
    ...
    def from_df(cls, df, thread_root_functions=None, ts_cols=('calltime', 'rettime')):
        ...
        def make_visitor():
            ...
            def visit(row):
                ...
                elif curr_event == event_enum.TAG:
                    tags = row['tags']
            ...
            def finalize():
                ...
                if curr_node is not root_node:
                    ...
                    curr_node.exit_time = last_time
                    for node in curr_node.parents:
...
class _CallGraphNode(Mapping):
    ...
    def __init__(self, func_name, parent, logical_thread, entry_time=None, exit_time=None, valid_metrics=True):
    ...
    def _inherited_tags(self):
        ...
        # The root has the lowest priority in tags definition in case of
        # conflict, to allow leaves to tag whole paths.
        parents = reversed(list(self.parents))
        nodes = chain(parents, self.indirect_children)
